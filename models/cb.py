...
def train_and_predict(X_train, X_valid, y_train, y_valid, X_test, cb_params, cat, oof):

    logging.debug(cb_params)

    if len(cat) >= 1:
        cb_train = Pool(X_train, label=y_train, cat_features=cat)
        cb_valid = Pool(X_valid, label=y_valid, cat_features=cat)
    else:
...
def save_importances(models, tr_cols, time, logging):
    ...
    logging.debug(f'low importance features: {list(unique_feature_importance_df[unique_feature_importance_df["importance"] <= 100].index)}')
